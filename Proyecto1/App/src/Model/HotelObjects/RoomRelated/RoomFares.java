...
public class RoomFares implements HotelObject {
    ...
    * - Para todos los días de la semana de la estadía debe existir una tarifa que lo cubra
    * <br>
    * @params initialDate Dia de inicio de la estadía
    * @params finalDate Dia en el que termina la estadía
    *
    * @throws Exception Las tarifas en el rango de la estadía están mal configuradas
    *
    */
        int fare = 0;
        ...
        while(actualDate.compareTo(finalDate) <= 0 && initialIndex < sortedFares.size()){
            ...
            if(fareDate.getInitialDate().compareTo(actualDate) <= 0 && fareDate.getFinalDate().compareTo(actualDate) >= 0){
        ...
        if(fare != 0)
            ...
            throw new Exception(String.format("La tarifa entre %s y %s está mal configurada", initialDate.toString(), finalDate.toString()));
    }
    ...
    private int searchIndexFloorInitialDate(ArrayList<Fare> fareList, LocalDate initialDate) {
        ...
        // Como retorna el valor estrictamente menor, si el siguiente elemento es la fecha exacta se retorna esa fecha, si no, la anterior
